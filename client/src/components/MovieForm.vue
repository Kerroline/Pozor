<template>
  <div>
    <form @submit.prevent>
      <label>Title</label>
      <my-input v-model="movieModel.title" type="text" placeholder="Title" />
      ...
      <select
        ...
        multiple
      >
        <option v-for="genre in selectGenres" :key="genre.id" :value="genre.id">
          {{ genre.name }}
        </option>
      </select>
      <p>
      ...
      <input type="file" @change="handlePosterUpload($event)" />
      <label>Select galleries</label>
      <input type="file" multiple @change="handleGalleriesUpload($event)" />
      <my-button
